...
while True:
    folder_to_rename = input("Which folder would you like to rename the files to lowercase? - Please type the full path: ")
...
# Loop through files and rename them lower case

for file in Path(folder_to_rename).iterdir():
    if file.is_file() and file.name not in ignored_files:
        new_name = file.stem.lower() + file.suffix
